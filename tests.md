...
6. Click `Review` and confirm the dropdown closes and the review pane opens without showing a `Run review` button; reopen the dropdown, click `Review` again, and confirm the dropdown closes and the pane toggles closed.
...
- Clicking the dropdown `Review` row always closes the dropdown after toggling the Review pane.
- The Review pane toolbar keeps `Refresh` but does not show a `Run review` button.
